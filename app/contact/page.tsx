...
import { Check, Calendar, Clock, Video, HelpCircle, ArrowRight, ChevronDown, ChevronUp, Search, Map, CheckCircle, Loader2, Lock } from "lucide-react";
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <input name="firstName" required onChange={handleChange} type="text" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-primary transition-colors" placeholder="Jane" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-bold text-zinc-300">Email</label>
                      <input name="email" required onChange={handleChange} type="email" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-primary transition-colors" placeholder="jane@company.com" />
                    </div>
                  </div>

                  <div className="space-y-2">
                    <label className="text-sm font-bold text-zinc-300">Website / Social URL</label>
                    <input name="website" onChange={handleChange} type="text" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-primary transition-colors" placeholder="www.yourwebsite.com" />
                  </div>

                  <div className="space-y-2">
                    <label className="text-sm font-bold text-zinc-300">Which best describes you?</label>
                    <select name="stage" onChange={handleChange} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-primary transition-colors appearance-none">
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea name="pain" onChange={handleChange} rows={3} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-primary transition-colors" placeholder="e.g. My email list doesn't talk to my course platform..."></textarea>
                  </div>

                  <div className="space-y-2">
                    <label className="text-sm font-bold text-zinc-300">Budget Range</label>
                    <select name="budget" onChange={handleChange} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-primary transition-colors appearance-none">
                  ...
                  <div className="pt-4">
                    ...
                    <button type="submit" disabled={status === "loading"} className="w-full btn-primary py-4 rounded-xl font-bold text-lg hover:shadow-primary/20 hover:shadow-lg transition-all flex justify-center items-center gap-2 disabled:opacity-70">
                      {status === "loading" ? <Loader2 className="w-6 h-6 animate-spin" /> : "Submit Application"}
                    </button>
                    <p className="text-center text-xs text-zinc-500 mt-4">After submitting, you can confirm your time on the calendar.</p>
                  </div>
                </form>
              )}
            </div>

            {/* OPTIMIZED CALENDAR SECTION */}
            <div className="bg-white relative h-[800px] lg:h-auto border-l border-grey-medium/10">
              {status === "success" ? (
                <iframe 
                  ...
                  className="absolute inset-0 w-full h-full border-0 animate-in fade-in zoom-in duration-500" 
                ...
                <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-zinc-50/50">
                   ...
                   <h4 className="text-xl font-bold text-zinc-900">Booking Calendar</h4>
                   <p className="text-zinc-500 max-w-xs mt-2 text-sm leading-relaxed">
                     Complete the form to view available slots. We use your application data to prepare for the call, allowing us to provide a more accurate and valuable audit during our session.
                   </p>
                </div>
              )}
            </div>
          ...
          <div className="grid gap-6">
            {faqs.slice(0, isFaqExpanded ? faqs.length : 3).map((faq, index) => (
              <div key={index} className="bg-white p-8 rounded-2xl border border-grey-medium/10 shadow-sm hover:shadow-md transition-all">
                ...
                <p className="text-grey-medium font-medium leading-relaxed pl-8">{faq.answer}</p>
              </div>
            ))}
          </div>
          <div className="mt-12 text-center">
            <button onClick={() => setIsFaqExpanded(!isFaqExpanded)} className="inline-flex items-center gap-2 text-primary font-bold text-lg hover:text-blue-700 transition-colors border-b-2 border-primary/20 hover:border-primary pb-1">
              {isFaqExpanded ? <>Show Less <ChevronUp className="w-4 h-4" /></> : <>Read More Questions <ChevronDown className="w-4 h-4" /></>}
            </button>
          </div>
        </div>
      </section>
